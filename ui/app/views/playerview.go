...
func (m *mediaPlayer) getProgress(media bluetooth.MediaData, buttons string, width int, skip bool) (string, string, string, string) {
	var length int
...
func (m *mediaPlayer) updateLoop(deviceName string) {
	...
	player, views := m.setup(deviceName)
	playerInfo := views[0]
	playerTitle := views[1]
	...
	m.app.InstantDraw(func() {
		m.help.swapStatusHelp(player, true)
	})
	defer m.app.InstantDraw(func() {
		m.help.swapStatusHelp(player, false)
	})
...
PlayerLoop:
	for {
		...
		title, buttons, tracknum, progress := m.getProgress(media, mediaButtons, width, m.getSkip())

		m.app.InstantDraw(func() {
			playerInfo.SetText(media.Artist + " - " + media.Album)
...
func (m *mediaPlayer) setup(deviceName string) (*tview.Flex, []*tview.TextView) {
...
func (m *mediaPlayer) getSkip() bool {
	m.Lock()
	defer m.Unlock()
...
func (m *mediaPlayer) setSkip(skip bool) {
	m.Lock()
	defer m.Unlock()
